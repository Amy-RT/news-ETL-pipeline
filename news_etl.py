...
# Task 6: Import additional libraries
from datetime import datetime, timedelta, time 
from airflow import DAG
from airflow.operators.python import PythonOperator

# Task 6: Initialize NewsApiClient
news_api_key = ""
news_api = NewsApiClient(api_key=news_api_key)

# Task 6: Assign datetime variables
to_date = datetime.utcnow().date()
from_date  = to_date - timedelta(days=1)

# Task 6: Intialize DAG object
dag = DAG(
     dag_id="news_etl",
...
# Task 7: Replace your function with the following extract_news_data function:
...
# Task 7: Replace your function with the following transform_news_data function:
def transform_news_data(**kwargs):
    ...
    logging.info("transform_news_data xcom pull successful.")
    ...
    logging.info("transform_news_data xcom push successful.")


# Task 7: Replace your function with the following load_news_data function:
def load_news_data(**kwargs):
    with sqlite3.connect("/usercode/etl_news_data.sqlite") as connection:
        ...
        logging.info("load_news_data xcom pull successful.")

        # Convert data into a DataFrame
        df = pd.read_json(data)
        
        # Logging message before loading data
        logging.info("load_news_data ready to load data from database.")

        df.to_sql(name="news_table", con=connection, index=False, if_exists="append")
        
        # Logging message after loading data
        logging.info("load_news_data successfully read from database.")

# Task 8: Create Python operators
_extract_news_data = PythonOperator(task_id="extract_news",
                                    python_callable=extract_news_data,
                                    provide_context=True,
                                    dag=dag)

_transform_news_data = PythonOperator(task_id="transform_news",
                                        python_callable=transform_news_data,
                                        provide_context=True,
                                        dag=dag)
_load_news_data = PythonOperator(task_id="load_news",
                                    python_callable=load_news_data,
                                    provide_context=True,
                                    dag=dag)


# Task 8: Create depedencies
